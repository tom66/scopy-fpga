...
void fabcfg_init()
{
 ...
 XGpioPs_SetDirectionPin(&g_hal.xgpio_ps, 3, 1);
 XGpioPs_SetOutputEnablePin(&g_hal.xgpio_ps, 3, 1);
 XGpioPs_SetDirectionPin(&g_hal.xgpio_ps, 4, 0);
...
{
 reg &= 0xfff;
 return Xil_In32(0x40000000U + (reg * 4));
}
...
{
 reg &= 0xfff;
 Xil_Out32(0x40000000U + (reg * 4), data);
}
...
void fabcfg_commit()
{
 ...
 XGpioPs_WritePin(&g_hal.xgpio_ps, 3, 1);
 bogo_delay(1);
 XGpioPs_WritePin(&g_hal.xgpio_ps, 3, 0);


 d_start_timing(15);
 while(timeout--) {
  if(XGpioPs_ReadPin(&g_hal.xgpio_ps, 4)) {
   break;
  }
 ...
 if(timeout == 0) {
  d_printf(4, "FabCfg: Timeout waiting for fabric to respond to COMMIT");
  exit(-1);
 }
}
