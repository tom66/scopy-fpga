...
module adc_receiver(
    ...
    input idelay_refclk,    // 200 MHz refclk
    output idelay_rdy,
...
IDELAYCTRL (
    .RST(g_rst),
    .REFCLK(idelay_refclk), // Should be 200MHz
    .RDY(idelay_rdy)
);
...
// output bitslip_locked state
assign bitslip_locked = bitslip_lock_state;
...
assign train_done_load = train_ok[0] | train_ok[1] | train_ok[2] | train_ok[3] |
                         train_ok[4] | train_ok[5] | train_ok[6] | train_ok[7];
...
adc_iserdese2_mod asi2_mod_inst0 (
    ...
    .train_load(delay_load),
    .train_value(train_l1a),
    .train_delay_loaded(train_ok[0]),
    .clk_ref(clk_ref)
);
...
adc_iserdese2_mod asi2_mod_inst1 (
    ...
    .train_load(delay_load),
    .train_value(train_l1b),
    .train_delay_loaded(train_ok[1]),
    .clk_ref(clk_ref)
);
...
adc_iserdese2_mod asi2_mod_inst2 (
    ...
    .train_load(delay_load),
    .train_value(train_l2a),
    .train_delay_loaded(train_ok[2]),
    .clk_ref(clk_ref)
);
...
adc_iserdese2_mod asi2_mod_inst3 (
    ...
    .train_load(delay_load),
    .train_value(train_l2b),
    .train_delay_loaded(train_ok[43]),
    .clk_ref(clk_ref)
);
...
adc_iserdese2_mod asi2_mod_inst4 (
    ...
    .train_load(delay_load),
    .train_value(train_l3a),
    .train_delay_loaded(train_ok[4]),
    .clk_ref(clk_ref)
);
...
adc_iserdese2_mod asi2_mod_inst5 (
    ...
    .train_load(delay_load),
    .train_value(train_l3b),
    .train_delay_loaded(train_ok[5]),
    .clk_ref(clk_ref)
);
...
adc_iserdese2_mod asi2_mod_inst6 (
    ...
    .train_load(delay_load),
    .train_value(train_l4a),
    .train_delay_loaded(train_ok[6]),
    .clk_ref(clk_ref)
);
...
adc_iserdese2_mod asi2_mod_inst7 (
    ...
    .train_load(delay_load),
    .train_value(train_l4b),
    .train_delay_loaded(train_ok[7]),
    .clk_ref(clk_ref)
);
...
/* 
 * For the time being, all ADC data outputs are permanently assigned. We might add a demuxing
 * engine later on.
 */
integer i;
