...
parameter FIFO_DATA_LOW_UNLAT   = FIFO_DATA_LOW + 256;  // Threshold to stop packing data into FIFO
parameter FIFO_DATA_MIPI_VALID  = 1022;                 // Should equal chosen MIPI line word size minus 2 words (allows last line to complete output)
...
wire csi_start_lines, csi_start_frame, csi_end_frame, csi_stop, csi_sleep, csi_clock_gating_en, csi_reset_fifo;
...
assign csi_reset_fifo = R_csi_ctrl_a[6];
assign csi_reset_periph = R_csi_ctrl_a[7];
...
always @(posedge s00_axis_aclk) begin
    if (fifo_data_count < FIFO_DATA_LOW) begin
        fifo_space_available <= 1;
    end else if (fifo_data_count > (FIFO_DATA_LOW_UNLAT + 128)) begin
...
// Output read valid signal indicating that a read is possible (at least 1022 words available)
...
fifo_generator_csi_gearbox (
    ...
    .rst((~s00_axis_aresetn) | csi_reset_fifo | csi_reset_periph),
